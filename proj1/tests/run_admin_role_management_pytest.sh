...
OUTPUT_FILE="$OUTPUT_DIR/admin-role-management-pytest-$TIMESTAMP.txt"

mkdir -p "$OUTPUT_DIR"

{
  echo "Use Case #11 pytest run: Administrative Role Management"
  echo "Started: $(date)"
  echo "API_BASE_URL: ${API_BASE_URL:-http://localhost:8080}"
  echo ""
} | tee "$OUTPUT_FILE"

"$PYTHON" -m pytest -vv "$SCRIPT_DIR/test_admin_role_management_pytest.py" 2>&1 | tee -a "$OUTPUT_FILE"
