...
OUTPUT_FILE="$OUTPUT_DIR/chat-assistant-pytest-$TIMESTAMP.txt"

mkdir -p "$OUTPUT_DIR"

{
  echo "Use Case #15 pytest run: AI Chat Assistant"
  echo "Started: $(date)"
  echo "API_BASE_URL: ${API_BASE_URL:-http://localhost:8080}"
  echo ""
} | tee "$OUTPUT_FILE"

"$PYTHON" -m pytest -vv "$SCRIPT_DIR/test_chat_assistant_pytest.py" 2>&1 | tee -a "$OUTPUT_FILE"
