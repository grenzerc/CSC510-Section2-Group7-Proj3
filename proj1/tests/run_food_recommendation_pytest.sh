...
OUTPUT_FILE="$OUTPUT_DIR/food-recommendation-pytest-$TIMESTAMP.txt"

mkdir -p "$OUTPUT_DIR"

{
  echo "Use Case #14 pytest run: Food Recommendation"
  echo "Started: $(date)"
  echo "API_BASE_URL: ${API_BASE_URL:-http://localhost:8080}"
  echo ""
} | tee "$OUTPUT_FILE"

"$PYTHON" -m pytest -vv "$SCRIPT_DIR/test_food_recommendation_pytest.py" 2>&1 | tee -a "$OUTPUT_FILE"
