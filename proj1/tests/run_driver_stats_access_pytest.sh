...
OUTPUT_FILE="$OUTPUT_DIR/driver-stats-access-pytest-$TIMESTAMP.txt"

mkdir -p "$OUTPUT_DIR"

{
  echo "Use Case #13 pytest run: Driver Delivery Statistics"
  echo "Started: $(date)"
  echo "API_BASE_URL: ${API_BASE_URL:-http://localhost:8080}"
  echo ""
} | tee "$OUTPUT_FILE"

"$PYTHON" -m pytest -vv "$SCRIPT_DIR/test_driver_stats_access_pytest.py" 2>&1 | tee -a "$OUTPUT_FILE"
