...
OUTPUT_FILE="$OUTPUT_DIR/inventory-menu-authorization-pytest-$TIMESTAMP.txt"

mkdir -p "$OUTPUT_DIR"

{
  echo "Use Case #12 pytest run: Inventory and Menu Management"
  echo "Started: $(date)"
  echo "API_BASE_URL: ${API_BASE_URL:-http://localhost:8080}"
  echo ""
} | tee "$OUTPUT_FILE"

"$PYTHON" -m pytest -vv "$SCRIPT_DIR/test_inventory_menu_authorization_pytest.py" 2>&1 | tee -a "$OUTPUT_FILE"
